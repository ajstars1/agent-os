    ...
    memory: async (args, ctx) => {
        ...
        if (subCmd === 'stats') {
            ...
            for (const s of stats) {
                const lastDate = s.lastAccessed
                    ...
                    : 'never';
                process.stdout.write(`${s.topic.slice(0, 19).padEnd(20)} ${String(s.accessCount).padEnd(10)} ${lastDate}\n`);
            }
            process.stdout.write('\n');
            return;
        }
        if (subCmd === 'add') {
            if (!ctx.hamStore || !ctx.hamCompressor) {
                process.stdout.write(ctx.hamStore
...
export async function handleCommand(input, ctx) {
