...
export async function bootstrap(config) {
    ...
    if (hamCompressor) {
        ingestSkillsToHAM(config.SKILLS_DIR, hamStore, hamCompressor, logger).catch((err) => {
            logger.warn({ err }, 'HAM skill ingestion failed');
        });
    }
    const engine = new AgentEngine(config, memory, skills, tools, claude, gemini, router, logger, hamRetriever, hamStore);
