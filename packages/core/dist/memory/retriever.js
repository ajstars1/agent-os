...
export class HAMRetriever {
    ...
    retrieve(userMessage, _history, conversationId) {
     ...
     */
    assembleMemory(activeTopic, depth) {
        const usedIds = [];
        const parts = [];
        // 1. All L0 headlines
        const all = this.store.getAllL0();
        if (all.length > 0) {
     ...
     */
    trimToTokenBudget(fullText, activeTopic) {
        ...
        for (const entry of sorted.reverse()) {
            if (entry.topic === activeTopic)
                continue; // already in active section
